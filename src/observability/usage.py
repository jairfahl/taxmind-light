...
def registrar_uso(
    ...
    try:
        ...
        cur.execute(
            """
            INSERT INTO api_usage (service, model, input_tokens, output_tokens, estimated_cost, tenant_id)
            VALUES (%s, %s, %s, %s, %s, %s)
            """,
            (service, model, input_tokens, output_tokens, custo, tenant_id),
        )
        conn.commit()
        cur.close()
        logger.debug(
            "Uso registrado: %s/%s input=%d output=%d custo=$%.6f tenant=%s",
            service, model, input_tokens, output_tokens, custo, tenant_id,
        )
